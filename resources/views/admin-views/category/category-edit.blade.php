                        ...
                        <form action="{{route('admin.category.update',[$category['id']])}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                @if($category['parent_id']==0)
                                    ...
                                    <div class="from_part_2">
                                        <label class="title-color pt-2">{{\App\CPU\translate('image')}}</label>
                                        
                                        <div class="custom-file text-left">
                                            <input type="file" name="image2" id="customFileEg1"
                                                   class="custom-file-input"
                                                   accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                                            <label class="custom-file-label"
                                                   for="customFileEg1">{{\App\CPU\translate('choose')}} {{\App\CPU\translate('image')}}</label>
                                        </div>
                                    </div>
                                    <div class="from_part_2">
                                        <label class="title-color pt-3" style="">{{\App\CPU\translate('discount')}}</label>
                                        
                                        <div class="custom-file text-left">
                                            <input type="number" name="discount" value="{{$category['discount']}}"
                                                class="form-control" >
                                           
                                        </div>
                                    </div>
                                    <div class="from_part_2">
                                        <label class="title-color pt-3" style="">{{\App\CPU\translate('discount type')}}</label>
                                        
                                        <div class="custom-file text-left">
                                            <select name="discount_type"
                                                class="form-control" >
                                                <option @if($category->discount_type == 'flat') selected @endif value="flat">flat</option>
                                                <option @if($category->discount_type == 'percent') selected @endif value="percent">percent</option>
                                            </select>
