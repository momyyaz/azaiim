...
class Helpers
{
    ...
    public static function get_product_discount_based_on_category_discount($category, $price)
    {
        $discount = 0;
        if ($category->discount_type == 'percent') {
            $discount = ($price * $category->discount) / 100;
        } elseif ($category->discount_type == 'flat') {
            $discount = $category->discount;
        }
        $price_after_discount = $price - $discount;
        

        return $price_after_discount >= 0 ? floatval($price_after_discount) : floatval(0);
    }
